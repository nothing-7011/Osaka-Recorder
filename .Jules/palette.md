...
## 2024-06-05 - [Copy Action for Overlay Windows]
**Learning:** `FLAG_NOT_FOCUSABLE` in Overlay windows (used for pass-through touches) prevents native text selection, frustrating users who expect to be able to copy displayed text.
**Action:** Always provide an explicit "Copy" button (using `LocalClipboardManager`) for text content displayed in `TYPE_APPLICATION_OVERLAY` windows.
